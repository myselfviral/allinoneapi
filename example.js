...
const client = new Client({ puppeteer: { headless: true , args: ['--no-sandbox'], ignoreDefaultArgs: ['--disable-extensions'] }, session: sessionCfg });
...
router.post('/init', async function(req, res) {
    ...
    const newClient = new Client({ puppeteer: {headless: true, args: ['--no-sandbox'],ignoreDefaultArgs: ['--disable-extensions'] }, session: sessionCfg });
    // You can use an existing session and avoid scanning a QR code by adding a "session" object to the client options.
    // This object must include WABrowserId, WASecretBundle, WAToken1 and WAToken2.
    try {
